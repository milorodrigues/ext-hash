...
int getHashFileSize(){
	...
	if (hashFile == NULL) {
		hashFile = fopen("hash.bin", "w+"); fclose(hashFile);
		return 0; }
	else while (fgetc(hashFile) != EOF) ++n;
...
int main(){
	...
	if (getHashFileSize() < 40){
			// 40 bytes é o tamanho do hash.bin quando o heap está vazio,
			// então é o menor tamanho possível
		//cout << "Hash file is empty, clearing all data...\n";
		clearAllData();
	}
...
void printFile(){
	...
	for (int i = leftLeaf; i <= rightLeaf; i++){
		...
		cout << key << " ";
		if (vPages[vTable[i].page].occupied == 0) cout << "null";
		else cout << (vTable[i].page + 1);
		cout << "\n";
	...
	for (int i=0;i<PAGE_NUM;i++){
		...
		if(vPages[i].occupied == 0) continue;
